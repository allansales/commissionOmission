...
class AuthorSpider(scrapy.Spider):
	...
	def parse_paired_args_by_multi_topics(self, response):
		...
		return {
			...
			'argument': self.preprocess_text(pro_args) + self.preprocess_text(con_args) # needs to remove the tags from the content (recreational marijuana)
		}
	...
	def parse_unpaired_args_page(self, response):
		return {
			...
			'argument': self.preprocess_text(response.xpath('//div[@class="argument-container"]//blockquote').getall()) # needs to remove the tags from the content (recreational marijuana)
		}
